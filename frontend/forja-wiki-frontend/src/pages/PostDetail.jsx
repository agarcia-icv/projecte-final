...
function PostDetail() {
  ...
  const fetchData = async () => {
    try {
      ...
      const postData =
        postRes.data.data ?? postRes.data

      setPost(postData)

      const myRating =
        postData.valoracions?.find(
      ...
      const avgRes =
        await api.get(
          `/posts/${id}/valoracio-mitjana`
        )

      setAverage(
        avgRes.data.valoracio_mitjana
      )
